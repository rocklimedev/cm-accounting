...
const rowsFromTitles = (titles) =>
  titles && titles.length ? titles.map((t) => newRow(t)) : [newRow()];
const blank = (title) => ({
  report_date: todayStr(),
  expenses: [newRow(title)],
  employee_remarks: "",
});
...
export default function ExpenseReportForm() {
        ...
        if (!isEdit) {
          ...
          if (saved) {
            try {
              ...
              const hasData =
                parsed.expenses &&
                parsed.expenses.some((r) => Number(r.amount) > 0);
              if (hasData) {
      ...
      api
        ...
        .then((res) => {
          ...
          setForm({
            ...
            expenses: (d.expenses && d.expenses.length
              ? d.expenses
              : [newRow()]
            ).map((e) => ({
              title: e.title || "",
  ...
  const addRow = () =>
    setForm((f) => ({ ...f, expenses: [...f.expenses, newRow(titles[0])] }));
  const removeRow = (i) =>
    setForm((f) => ({
      ...f,
  ...
  const summary = useMemo(() => {
    ...
    form.expenses.forEach((r) => {
      const a = n(r.amount);
      if (a > 0) s.count += 1;
      s.total += a;
      const m = r.payment_mode || "cash";
      if (s[m] !== undefined) s[m] += a;
    });
    return s;
  }, [form.expenses]);
  ...
  const save = async (status) => {
    ...
    try {
      ...
      const res = isEdit
        ? await api.put(`/expense-reports/${reportId}`, payload)
  ...
  return (
    <Layout
      title={isEdit ? `Edit Expense Report ${reportId}` : "Expense Report"}
    >
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card className="border border-border rounded-md p-4 sm:p-5 bg-card shadow-none">
            ...
            <p className="text-xs text-foreground/60 mb-3">
              ...
              payment mode for the ones you incurred — leave the rest at 0. Only
              rows with an amount are saved.
            </p>
            {errors.expenses && (
              <p
                    ...
                    <TableRow key={i}>
                      ...
                      <TableCell>
                        <Select
                          ...
                          onValueChange={(v) => setRow(i, "payment_mode", v)}
                        >
                          <SelectTrigger
                            className="h-9"
