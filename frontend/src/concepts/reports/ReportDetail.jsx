...
export default function ReportDetail() {
  ...
  const addRemark = async () => {
    ...
    setBusy(true);
    try {
      await api.post(`/${EDIT_PATH[report.report_type]}/${reportId}/remark`, {
        remark,
      });
  ...
  return (
    <Layout title={`Report ${reportId}`}>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div>
              ...
              <div className="text-sm text-foreground/60 mt-2 grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-1">
                ...
                <span>
                  Created By:{" "}
                  <b className="text-foreground">{report.created_by_name}</b>
                </span>
                <span>
                  Submitted By:{" "}
                  <b className="text-foreground">{report.submitted_by_name}</b>
                </span>
                <span>
                  Submitted:{" "}
                  <b className="text-foreground">
                ...
                {report.last_updated_by && (
                  <span>
                    Last Updated By:{" "}
                    <b className="text-foreground">{report.last_updated_by}</b>
                  </span>
                )}
                {report.last_updated_at && (
                  <span>
                    Last Updated:{" "}
                    <b className="text-foreground">
                      {formatDateTime(report.last_updated_at)}
                    </b>
                  </span>
                )}
              </div>
              ...
              {adminRemarks.map((r, i) => (
                ...
                >
                  ...
                  <div className="text-xs text-foreground/50 mt-1">
                    by {r.added_by} &middot; {formatDateTime(r.at)}
                  </div>
                </div>
              ))}
            </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add Admin Remark &mdash; {reportId}</DialogTitle>
          </DialogHeader>
          <Textarea
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <DetailCard title="Retail by Payment Mode">
        ...
        <KV
          label="Total Retail"
          value={formatMoney(report.total_retail)}
          bold
        />
      </DetailCard>
    </div>
    ...
    <div className="space-y-4">
                ...
                <TableRow key={i}>
                  ...
                  <TableCell>
                    {e.entry_type === "debtor_received"
                      ? PAY_LABEL[e.payment_mode] || e.payment_mode
                      : "-"}
                  </TableCell>
                </TableRow>
              ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <MiniCard
          ...
          value={formatMoney(report.new_debtor_total)}
        />
        <MiniCard
          label="Debtor Received"
          value={formatMoney(report.debtor_received_total)}
        />
        <MiniCard
          label="Closing Debtor"
          value={formatMoney(run.closing)}
          accent
        />
      </div>
    </div>
...
function MiniCard({ label, value, accent }) {
  return (
    <Card
      className={`border border-border rounded-md p-3 bg-card shadow-none ${accent ? "border-l-2 border-l-primary" : ""}`}
    >
