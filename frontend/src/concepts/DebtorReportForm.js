...
import { Plus, Trash2, Loader2, AlertTriangle, Save, Send, HandCoins, ArrowDownToLine } from "lucide-react";
import { toast } from "sonner";

const PAY_MODES = [{ k: "cash", l: "Cash" }, { k: "upi", l: "UPI" }, { k: "bank", l: "Bank" }, { k: "card", l: "Card" }];
const newRow = (type) => (type === "debtor_received" ? { entry_type: "debtor_received", amount: "", payment_mode: "cash" } : { entry_type: "new_debtor", amount: "" });
const blank = () => ({ report_date: todayStr(), entries: [newRow("new_debtor")], employee_remarks: "" });
...
export default function DebtorReportForm() {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      api.get(`/debtor-reports/${reportId}`).then((res) => {
        const d = res.data;
        setReportStatus(d.status || "draft");
        setForm({
          report_date: (d.report_date || todayStr()).slice(0, 10),
          entries: (d.entries && d.entries.length ? d.entries : [newRow("new_debtor")]).map((e) => ({
            entry_type: e.entry_type, amount: e.amount ?? "", payment_mode: e.payment_mode || "cash",
          })),
  ...
  const setEntry = (i, key, val) => setForm((f) => {
    const rows = [...f.entries]; rows[i] = { ...rows[i], [key]: val }; return { ...f, entries: rows };
  });
  const addRow = (type) => setForm((f) => ({ ...f, entries: [...f.entries, newRow(type)] }));
  const removeRow = (i) => setForm((f) => ({ ...f, entries: f.entries.filter((_, idx) => idx !== i) }));

  const calc = useMemo(() => {
    let nw = 0, rc = 0;
    form.entries.forEach((e) => { if (e.entry_type === "new_debtor") nw += n(e.amount); else rc += n(e.amount); });
  ...
  const validate = () => {
    ...
    if (overReceived) e.received = "Debtor Received cannot exceed the current Outstanding Debtor";
  ...
  const buildPayload = (status) => ({
    ...
    entries: form.entries.map((e) => e.entry_type === "debtor_received"
      ? { entry_type: "debtor_received", amount: n(e.amount), payment_mode: e.payment_mode }
  ...
  const save = async (status) => {
    ...
    try {
      ...
      const res = isEdit ? await api.put(`/debtor-reports/${reportId}`, payload) : await api.post("/debtor-reports", payload);
      localStorage.removeItem(storageKey);
      toast.success(status === "submitted" ? "Debtor report submitted" : (isEdit ? "Report updated" : "Draft saved"));
  ...
  if (!loaded) return <Layout title="Debtor Report"><div className="text-sm text-foreground/50">Loading...</div></Layout>;
          ...
          <Section title="Basic Information">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field label="Current Outstanding Debtor">
                <Input value={formatMoney(calc.opening)} disabled data-testid="field-current-outstanding" className="font-semibold" />
              </Field>
            </div>
          </Section>
          ...
          <Card className="border border-border rounded-md p-4 sm:p-5 bg-card shadow-none">
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-border">
              ...
              <div className="flex gap-2">
                <Button type="button" variant="outline" size="sm" onClick={() => addRow("new_debtor")} data-testid="add-new-debtor-button">
                  <HandCoins className="h-4 w-4 mr-1" /> New Debtor
                </Button>
                <Button type="button" variant="outline" size="sm" onClick={() => addRow("debtor_received")} data-testid="add-debtor-received-button">
                ...
                <div key={i} className="border border-border rounded-sm p-3">
                  <div className="flex flex-wrap items-end gap-3">
                    <div className="space-y-1">
                      ...
                      <Select value={en.entry_type} onValueChange={(v) => setEntry(i, "entry_type", v)}>
                        <SelectTrigger className="h-9 w-44" data-testid={`entry-type-${i}`}><SelectValue /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-medium">{en.entry_type === "debtor_received" ? "Amount Received" : "New Debtor Amount"}</label>
                      <div className="w-40">
                        <RupeeInput value={en.amount} onChange={(v) => setEntry(i, "amount", v)} testId={`entry-amount-${i}`} error={errors[`amt_${i}`]} />
                      </div>
                    </div>
                    {en.entry_type === "debtor_received" && (
                      <div className="space-y-1">
                        <label className="text-xs font-medium">Payment Mode</label>
                        <Select value={en.payment_mode} onValueChange={(v) => setEntry(i, "payment_mode", v)}>
                          <SelectTrigger className="h-9 w-36" data-testid={`entry-mode-${i}`}><SelectValue /></SelectTrigger>
                    ...
                    <div className="ml-auto">
                      {form.entries.length > 1 && (
                        <Button type="button" variant="outline" size="icon" className="h-9 w-9 text-primary border-primary" onClick={() => removeRow(i)} data-testid={`remove-entry-${i}`}>
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      )}
                    </div>
                  </div>
                  {en.entry_type === "new_debtor" && <div className="text-xs text-foreground/55 mt-2">No payment received \u2014 increases Outstanding Debtor only.</div>}
                </div>
              ))}
            </div>
            {errors.received && <div className="text-xs text-primary mt-3 flex items-center gap-1" data-testid="received-error"><AlertTriangle className="h-3.5 w-3.5" /> {errors.received}</div>}
          </Card>
        ...
        <div className="lg:col-span-1">
          <div className="lg:sticky lg:top-20 space-y-4">
            <Card className="border border-border rounded-md p-4 bg-card shadow-none">
              ...
              <SumRow label="New Debtor Added" value={formatMoney(calc.newTotal)} />
              <SumRow label="Debtor Received" value={formatMoney(calc.recvTotal)} />
              <div className="mt-2 pt-2 border-t border-border flex items-center justify-between">
                <span className="font-semibold">Closing Debtor</span>
                <span className={`text-lg font-bold tabular-nums ${overReceived ? "text-primary" : ""}`} data-testid="calc-closing-debtor">{formatMoney(calc.closing)}</span>
              </div>
            </Card>
            <div className="flex flex-col gap-2">
              <Button onClick={() => { if (validate()) setConfirmOpen(true); else toast.error("Please fix the highlighted fields"); }} disabled={saving} data-testid="debtor-report-submit-button">
                <Send className="h-4 w-4 mr-2" /> {adminEditing ? "Save Changes" : "Preview & Submit"}
              </Button>
              {!adminEditing && (
                <Button variant="outline" onClick={() => save("draft")} disabled={saving} data-testid="debtor-report-save-draft-button">
                  {saving ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Save className="h-4 w-4 mr-2" />} Save as Draft
                </Button>
              )}
              <Button variant="outline" onClick={() => navigate(-1)} data-testid="debtor-report-cancel-button">Cancel</Button>
            </div>
          </div>
        </div>
      </div>
